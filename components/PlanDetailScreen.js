...
export default function PlanDetailScreen({ route, navigation }) {
  ...
  // Extraemos días y horarios
  const diasUnicos = Array.from(new Set(plan.horarios.map(h => h.dia)));
  const diasStr = diasUnicos.join(', ');

  return (
    <ScrollView style={styles.container}>
      <Image source={{ uri: plan.imagenUrl }} style={styles.image} />
      <View style={styles.content}>
        <Text style={styles.title}>{plan.nombre}</Text>
        <Text style={styles.line}>📍 {plan.localizacion}</Text>
        <Text style={styles.line}>💶 {plan.precio.toFixed(2)} €</Text>
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 18, fontWeight: '600', marginTop: 12, marginBottom: 4 },
